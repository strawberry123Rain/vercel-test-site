...
import { X, Calendar, Clock, User, Building, AlertCircle } from 'lucide-react'
...
export default function KPIDetailModal({
  ...
  const renderMaintenanceItem = (plan: MaintenancePlan) => (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div className="flex items-center gap-2 text-gray-600">
              <Building size={14} />
              <span>Fastighet: {plan.property_id}</span>
            </div>
            <div className="flex items-center gap-2 text-gray-600">
              <Calendar size={14} />
              <span>Nästa datum: {plan.next_due_date ? formatDate(plan.next_due_date) : 'Ej satt'}</span>
            </div>
            <div className="flex items-center gap-2 text-gray-600">
              <Clock size={14} />
              <span>Frekvens: {plan.frequency}</span>
            </div>
            <div className="flex items-center gap-2 text-gray-600">
              <User size={14} />
              <span>Tilldelad: {plan.assigned_to || 'Ingen'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <span className="text-sm text-gray-600">{plan.estimated_duration_hours}h</span>
        </div>
      </div>
    </div>
  )
